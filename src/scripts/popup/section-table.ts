...
interface INodeCollection {
    ...
    inputUBB?: HTMLInputElement;
    inputMarkdown?: HTMLInputElement;
}

export class SectionTable {
    item: WB.AssignedPackedItem;
    domNodes: INodeCollection;
    objectURL: string;
    sectionEvent: SectionEvent;

    constructor(item: WB.AssignedPackedItem) {
        this.item = item;
        this.domNodes = {};
        this.objectURL = null;
    ...
    generateTable() {
        ...
        const fragment = SectionTable.importNode();
     ...
     * @public
     */
    repaint(item: WB.AssignedPackedItem): boolean {
        if (item && item.URL) {
            ...
            if (item.fullDirectoryPath) {
                ...
                this.domNodes.imageHolder.title = `图片文件：${file.name}`;
            }
            return true;
        } else {
            return false;
