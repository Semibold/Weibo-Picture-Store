...
微博图片的用户信息卡功能，可以查看微博图片PO主的相关信息。此功能默认关闭，可在扩展选项中开启。快捷键：`Esc` 键关闭当前窗口中的微博信息卡，按住 `Alt` 键可以禁止微博信息卡更新位置。另外可以使用默认快捷 `Alt+Shift+C`（可在 chrome://extensions/shortcuts 键盘快捷键中修改按键）来禁用/启用当前域名下的微博信息卡功能。
