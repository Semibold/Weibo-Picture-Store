...
export class Base64 {
  ...
  /**
   * @public
   * @alias Utils.textFromBuffer(buffer, true);
   * @param {ArrayBufferLike} buffer
   * @return {string}
   */
  static fromBuffer(buffer) {
    ...
    for (let i = 0; i < tripletLength; i += maxChunkLength) {
      parts.push(Base64.encodeChunk(bv, i, (i + maxChunkLength) > tripletLength ? tripletLength : (i + maxChunkLength)));
    }
  ...
  /**
   * @public
   * @alias Utils.bufferFromText(b64, true);
   * @param {string} b64
   * @return {Uint8Array}
   */
  static toBuffer(b64) {
